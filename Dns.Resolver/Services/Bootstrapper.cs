...
namespace Dns.Resolver.Services
{
	public class Bootstrapper
	{
		...
		{
			var whiteDomains = await GetWhiteDomains().ConfigureAwait(false);
			var blackDomains = await GetBlackDomains().ConfigureAwait(false);

			var stopWatch = new Stopwatch();
			stopWatch.Start();
			var resolvedWhiteDomains = await _resolve.ResolveDomainsWithRetry(whiteDomains, 3).ConfigureAwait(false);
			stopWatch.Stop();
			...
			stopWatch.Restart();
			var resolvedBlackDomains = await _resolve.ResolveDomainsWithRetry(blackDomains, 3).ConfigureAwait(false);
			...
			await _redis.SaveResolvedDomains(RedisKeys.WHITE_DOMAINS_RESOLVED, resolvedWhiteDomains).ConfigureAwait(false);
			await _redis.SaveResolvedDomains(RedisKeys.BLACK_DOMAINS_RESOLVED, resolvedBlackDomains).ConfigureAwait(false);
		}

		public async Task NotifyCompletion()
		{
			await _redis.Publish(RedisKeys.RESOLVE_COMPLETE_CHANNEL, DateTimeOffset.UtcNow.ToString("o")).ConfigureAwait(false);
		}

		private async Task<List<string>> GetWhiteDomains() =>
			await _dnsReadOnly.WhiteDomains.Select(x => x.Domain).ToListAsync().ConfigureAwait(false);

		private async Task<List<string>> GetBlackDomains()
		{
			var domains = await _redis.GetStringSetMembers(RedisKeys.BLACK_DOMAINS).ConfigureAwait(false);
